...
st.image("fang_worm.png", caption="Based off the Wright-Fisher model of evolution with genetic drift! (Image generated by ChatGPT)", use_column_width=True)
st.write("You are a humble worm farmer in the remote island nation of Wormtopia! To your dismay, due to a mutation at the T (or 'Psychokinesis') locus, local worms are developing the uncanny ability to alter reality with only their minds.")
st.write("Is this mutation going to spread until every worm in Wormtopia knows your inner monologue? Unfortunately, Wormtopia lacks any large-scale research facilities to help answer this question. Armed with your knowledge of worm evolutionary dynamics and the Wright-Fisher model of evolution with genetic drift, you take matters into your own hands.")
class WrightFisher:
    def __init__(self):
        self.AA_freqs  = []
        self.Aa_freqs  = []
...
# User inputs
st.write("We denote the mutated version of the Psychokinesis locus as 'P', and the wild type version of the Psychokinesis locus as 'p'. Before we can predict its trajectory, we have to approximate the current frequency of the 'P' allele.")
a0 = st.slider("Initial allele frequency of P", 0.01, 0.99, 0.5)

st.write("Are you modeling the entire worm population of Wormtopia or just your personal breeding stock? Due to genetic drift, mutations often spread faster in small populations than in large ones. Genetic drift describes the random changes in population allele frequencies that can be attributed to dumb luck.")
...
st.write("For any given P allele, what are the odds that it mutates into a p allele (or vice versa)? Mutations are hard to come by, so this number is likely going to be pretty tiny.")
...
st.pyplot(fig)
st.write(f"After {generations} generations, {aa_freqs[generations]} of the population has a pp genotype, {Aa_freqs[generations]} of the population has a Pp genotype, and {AA_freqs[generations]} of the population has a PP genotype.")